...
def add_cart(request, product_id):
    ...
    if is_cart_item_exists:
        ...
        for item in cart_item:
            existing_variation = item.variations.all()  # get all variations of the cart item
            ex_var_list.append(list(existing_variation))  # convert to list and append to ex_var_list
            id.append(item.id)  # get the id of the cart item
        print(ex_var_list)

        if product_variation in ex_var_list:
            index = ex_var_list.index(product_variation)
            item_id = id[index]  # get the id of the cart item that matches the product variations
            item = CartItem.objects.get(product=product,id=item_id)  # get the cart item
            item.quantity += 1  # increase the quantity of the cart item
            item.save()
        else:
